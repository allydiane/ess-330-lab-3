...
ggplot(meta) + borders("state", fill = "lightblue", colour = "white") + 
...
p_cases <- ggplot(cases_meta) + 
  ...
  geom_point(aes(x = wmX_cases, y = wmY_cases, size = cases), color = "red", aplha = 0.5) + 
  theme_linedraw() + 
  labs(title = "Weighted Center of COVID Cases", x = "", y = "", size = "Cases") + 
  theme(legend.position = "none")

p_deatha <- ggplot(deaths_meta) + 
  borders("state", fill = "lightblue", colour = "white") + 
  geom_point(aes(x = wmX_deaths, y = wmY_deaths, size = deaths), color = "red", aplha = 0.5) + 
  theme_linedraw() + 
